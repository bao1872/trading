...
def add_path_quality_labels(
    ...
    """
    生成路径质量标签。
    这些标签只描述买点后的价格路径质量，不把固定持有期当作卖点依据。
    horizon 仅作为观察窗口上限，用于离线研究样本是否给出正向确认。
    """
    out = df.copy()
    close = out["close"].to_numpy(float)
...
def build_bucket_profiles(events: pd.DataFrame, features: Sequence[str], out_path: str) -> pd.DataFrame:
    ...
    for feat in features:
        if feat not in events.columns:
            continue
        keep = [c for c in [feat, "quality_trade_score", "good_path_main", f"path_mfe_{PATH_QUALITY_HORIZON}", f"path_mae_{PATH_QUALITY_HORIZON}", f"path_efficiency_{PATH_QUALITY_HORIZON}", f"hit_5pct_before_stop_{PATH_QUALITY_HORIZON}", f"bars_to_3pct_{PATH_QUALITY_HORIZON}", "ret_20", "max_dd_20"] if c in events.columns]
        ...
        if "ret_20" in sub.columns:
            agg_map["score_ret_20"] = ("ret_20", "mean")
        ...
        for _, row in grp.iterrows():
            rows.append({
                ...
                "score_ret_20": round(float(row.get("score_ret_20", np.nan)), 5) if pd.notna(row.get("score_ret_20", np.nan)) else np.nan,
...
def main() -> None:
    ...
    bucket_features = [
        "dsa_trade_pos_01", "bb_pos_01", "bb_width_norm", "bb_expand_streak", "bars_since_dir_change",
        "rope_pivot_pos_01", "score_width_total", "score_trend_total", "score_setup_total",
        "w_dsa_trade_pos_01", "w_dsa_signed_vwap_dev_pct", "ret_5",
    ]
    ...
    result_tables["20_combo_scan"] = combo_df
    result_tables["20b_combo_top_events"] = combo_top_df
