...
PG_TABLE_DEFINITIONS: Dict[str, str] = {}


def _to_pg_sql(sql: str) -> str:
    """将 SQLite DDL 转换为 PostgreSQL DDL"""
    sql = re.sub(r'INTEGER PRIMARY KEY AUTOINCREMENT', 'BIGSERIAL PRIMARY KEY', sql)
    sql = re.sub(r'\bAUTOINCREMENT\b', 'GENERATED BY DEFAULT AS IDENTITY', sql)
    return sql


def _get_pg_table_def(table_name: str) -> str:
    """获取 PostgreSQL 版本的表定义"""
    if table_name not in TABLE_DEFINITIONS:
        raise ValueError(f"未知的表名：{table_name}")
    return _to_pg_sql(TABLE_DEFINITIONS[table_name])
...
def get_create_sql(table_name: str, dialect: str = None) -> str:
    """获取指定表的创建 SQL

    Args:
        table_name: 表名
        dialect: 'sqlite'/'postgresql'，None 时自动从 DATABASE_URL 检测
    """
    if table_name not in TABLE_DEFINITIONS:
        raise ValueError(f"未知的表名：{table_name}")
