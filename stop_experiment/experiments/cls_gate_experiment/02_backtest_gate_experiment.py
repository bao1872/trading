...
# -*- coding: utf-8 -*-
"""
sell_cls / buy_cls 门槛回测对比实验（8 组方案）

Purpose:
    验证 sell_cls 入场门控 + buy_cls 退出门控在端到端回测中的效果。
...
Outputs:
    - results/backtest/gate_backtest_comparison.csv

How to Run:
    python -m stop_experiment.experiments.cls_gate_experiment.02_backtest_gate_experiment
...
from stop_experiment.pipeline.stop_config import (
    OUTPUT_DIR, MODELS_DIR, OBS_VAL_END, BUY_COST, SELL_COST,
)
...
EXPERIMENT_CONFIGS = [
    {"label": "A_baseline", "entry_gate_sell_cls": None, "exit_threshold": 0.70},
    {"label": "B1_entry_s06", "entry_gate_sell_cls": 0.6, "exit_threshold": 0.70},
    {"label": "B2_entry_s07", "entry_gate_sell_cls": 0.7, "exit_threshold": 0.70},
    {"label": "B3_entry_s08", "entry_gate_sell_cls": 0.8, "exit_threshold": 0.70},
    {"label": "C1_exit_b06", "entry_gate_sell_cls": None, "exit_threshold": 0.60},
    {"label": "C2_exit_b08", "entry_gate_sell_cls": None, "exit_threshold": 0.80},
    {"label": "D1_combo_s07_b07", "entry_gate_sell_cls": 0.7, "exit_threshold": 0.70},
    {"label": "D2_combo_s08_b08", "entry_gate_sell_cls": 0.8, "exit_threshold": 0.80},
]
...
def run_backtest(
    ...
    pred_lookup, max_stocks=10, exit_threshold=0.70,
    ...
    if entry_gate_sell_cls is not None:
        signals_df = signals_df[signals_df["pred_sell_cls"] > entry_gate_sell_cls].copy()
    ...
    return {
        ...
        "empty_pool_days": empty_pool_days,
    }


def main():
    print("=" * 60)
    print("sell_cls / buy_cls 门槛回测对比实验")
    print("=" * 60)
    ...
    for config in EXPERIMENT_CONFIGS:
        label = config["label"]
        entry_gate = config["entry_gate_sell_cls"]
        exit_th = config["exit_threshold"]

        print(f"\n  --- {label} (entry_gate_sell_cls={entry_gate}, exit_threshold={exit_th}) ---")

        result = run_backtest(
            test_entry, price_pivot, trading_days, prev_close_map,
            pred_lookup, max_stocks=10,
            exit_threshold=exit_th,
            entry_gate_sell_cls=entry_gate,
        )
        ...
        print(f"    NAV={s['final_nav']:.4f}, Sharpe={s['sharpe']:.2f}, "
              ...
              f"交易数={s['n_trades']}, 空池天数={result.get('empty_pool_days', 0)}")
        print(f"    退出原因: model_risk={reasons.get('model_risk',0)}, "
              f"stop_loss={reasons.get('stop_loss',0)}, max_hold={reasons.get('max_hold',0)}")

    comp_df = pd.DataFrame(results_rows)
    comp_path = os.path.join(BACKTEST_DIR, "gate_backtest_comparison.csv")
    ...
    print(f"  {'方案':22s} {'NAV':>8s} {'Sharpe':>8s} {'MDD':>8s} {'胜率':>8s} {'交易':>5s} {'空池':>4s} "
          f"{'model_risk':>10s} {'stop_loss':>9s} {'max_hold':>8s}")
    for row in results_rows:
        print(f"  {row['label']:22s} {row['final_nav']:>8.4f} {row['sharpe']:>8.2f} "
              ...
              f"{row['n_model_risk']:>10d} {row['n_stop_loss']:>9d} {row['n_max_hold']:>8d}")
