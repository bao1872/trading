...
# -*- coding: utf-8 -*-
"""
buy_reg 低值反转回测对比实验（8 组方案）

Purpose:
    在端到端回测中验证 buy_reg 低值策略的实际收益。
    通过修改 score 列和入场过滤实现不同选股策略，
    复用 decide_eod() 保证退出决策口径一致。
...
Outputs:
    - results/backtest/buy_reg_low_backtest_comparison.csv

How to Run:
    python -m stop_experiment.experiments.buy_reg_low_experiment.02_backtest_buy_reg_low
...
EXPERIMENT_CONFIGS = [
    {"label": "A_baseline", "score_mode": "sell_reg", "entry_gate": None},
    {"label": "B1_buy_reg_close_zero", "score_mode": "buy_reg", "entry_gate": None},
    {"label": "B2_neg_buy_reg_very_neg", "score_mode": "neg_buy_reg", "entry_gate": None},
    {"label": "C1_composite_0.3_0.7", "score_mode": "composite_0.3_0.7", "entry_gate": None},
    {"label": "C2_buy_reg_abs", "score_mode": "neg_buy_reg_abs", "entry_gate": None},
    {"label": "D1_gate_br_gt_003", "score_mode": "sell_reg", "entry_gate": "buy_reg>-0.03"},
    {"label": "D2_gate_br_gt_005", "score_mode": "sell_reg", "entry_gate": "buy_reg>-0.05"},
    {"label": "E1_gate_br_plus_br_score", "score_mode": "buy_reg", "entry_gate": "buy_reg>-0.05"},
]
...
def apply_score_mode(signals_df: pd.DataFrame, score_mode: str) -> pd.DataFrame:
    df = signals_df.copy()
    if score_mode == "sell_reg":
        df["score"] = df["pred_sell_reg"]
    elif score_mode == "neg_buy_reg":
        df["score"] = -df["pred_buy_reg"]
    elif score_mode == "buy_reg":
        df["score"] = df["pred_buy_reg"]
    elif score_mode == "neg_buy_reg_abs":
        df["score"] = -np.abs(df["pred_buy_reg"])
    elif score_mode == "composite_0.3_0.7":
        df["score"] = df["pred_sell_reg"] * 0.3 + (-df["pred_buy_reg"]) * 0.7
    else:
        df["score"] = df["pred_sell_reg"]
    return df


def apply_entry_gate(signals_df: pd.DataFrame, entry_gate: str | None) -> pd.DataFrame:
    if entry_gate is None:
        return signals_df
    df = signals_df.copy()
    if entry_gate == "buy_reg>-0.03":
        df = df[df["pred_buy_reg"] > -0.03]
    elif entry_gate == "buy_reg>-0.05":
        df = df[df["pred_buy_reg"] > -0.05]
    elif entry_gate == "buy_reg<-0.10":
        df = df[df["pred_buy_reg"] < -0.10]
    return df
...
def run_backtest(signals_df, price_pivot, trading_days, prev_close_map,
                 pred_lookup, max_stocks=10, score_mode="sell_reg", entry_gate=None):

    signals_df = apply_entry_gate(signals_df, entry_gate)
    signals_df = apply_score_mode(signals_df, score_mode)
...
def main():
    print("=" * 60)
    print("buy_reg 低值反转回测对比实验（8 组方案）")
    print("=" * 60)
    ...
    for config in EXPERIMENT_CONFIGS:
        ...
        print(f"\n  --- {label} (score={score_mode}, gate={entry_gate}) ---")

        result = run_backtest(
            test_entry, price_pivot, trading_days, prev_close_map,
            pred_lookup, max_stocks=10,
            score_mode=score_mode, entry_gate=entry_gate,
        )
        ...
        row = {
            "label": label, "score_mode": score_mode, "entry_gate": str(entry_gate),
    ...
    comp_path = os.path.join(BACKTEST_DIR, "buy_reg_low_backtest_comparison.csv")
    ...
    print("回测对比汇总")
    print(f"{'='*110}")
    print(f"  {'方案':28s} {'评分':20s} {'门控':16s} {'NAV':>8s} {'Sharpe':>8s} {'MDD':>8s} {'胜率':>8s} {'交易':>5s} {'空池':>4s}")
    for row in results_rows:
        print(f"  {row['label']:28s} {row['score_mode']:20s} {row['entry_gate']:16s} "
              f"{row['final_nav']:>8.4f} {row['sharpe']:>8.2f} "
    ...
    for row in results_rows[1:]:
        ...
        print(f"    {row['label']:28s} ΔNAV={delta:+.4f} ({delta_pct:+.1f}%)")
