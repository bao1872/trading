...
How to Run:
    python dsa_experiment/pipeline/01_selection_dsa.py --ts-code 600547
    python dsa_experiment/pipeline/01_selection_dsa.py --date 2026-04-10
    python dsa_experiment/pipeline/01_selection_dsa.py --backfill --start-date 2024-01-01 --end-date 2026-04-30
Side Effects: 写入 stock_dsa_vreversal_results 表

管线位置: Step 1/7 — 周线触发点检测（整个流程起点）
...
def ensure_table_exists():
    ...
    alter_sqls = [
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS vol_zscore_5 FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS vol_zscore_10 FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS vol_zscore_20 FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS vol_ratio_10 FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS vol_stage_cv FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS vol_prev_stage_cv FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS vol_cv_ratio FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS price_vol_coord FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS momo_vol_coord FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS low_pos_break_coord FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS coord_consistency FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS coord_stage_current FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS coord_stage_prev FLOAT",
        f"ALTER TABLE {SELECTION_TABLE} ADD COLUMN IF NOT EXISTS coord_stage_ratio FLOAT",
    ]
    with engine.connect() as conn:
