...
How to Run:
    python dsa_experiment/experiments/analyze_daily_factors.py
Examples:
    python dsa_experiment/experiments/analyze_daily_factors.py
...
# 实验性子集：从 SSOT ALL_DAILY_FACTOR_COLS 中选取的分析分组
FACTOR_COLS_24 = [
    c for c in ALL_DAILY_FACTOR_COLS
    if c not in {
...
EXTRA_FACTOR_COLS = [
    c for c in ALL_DAILY_FACTOR_COLS
    if c not in set(FACTOR_COLS_24)
    and c not in {
        "price_vol_coord", "momo_vol_coord", "low_pos_break_coord",
        "coord_consistency",
        "coord_stage_current", "coord_stage_prev", "coord_stage_ratio",
